...
const validateAttendees = async (attendees) => {
  ...
  for (const attendee of attendees) {
    const input = typeof attendee === "string" ? attendee : attendee.email || attendee.name;
    if (!input) continue;
    const normalizedSearch = input.trim().toLowerCase().replace(/\s+/g, "");

    const isGroupReference = ["team", "group", "cluster"].some((kw) => normalizedSearch.includes(kw)) ||
      (typeof attendee === "object" && attendee.isTeam);

    if (isGroupReference) {
      const cluster = await Cluster.findOne({ name: { $regex: new RegExp(input, "i") } });
      if (cluster && cluster.members && cluster.members.length > 0) {
        const groupMembers = await Employee.find({ empId: { $in: cluster.members } }).select("email name");
        for (const member of groupMembers) {
          if (!foundAttendees.includes(member.email.toLowerCase())) foundAttendees.push(member.email.toLowerCase());
        }
        continue;
      }
    }
    ...
    if (EMAIL_REGEX.test(input)) {
      if (!foundAttendees.includes(input.toLowerCase())) foundAttendees.push(input.toLowerCase());
      continue;
    }
    ...
    if (!user) {
      if (typeof attendee === "object" && attendee.email) {
        if (!foundAttendees.includes(attendee.email.toLowerCase())) foundAttendees.push(attendee.email.toLowerCase());
        continue;
      }
      throw new Error(`User or group "${input}" not found. Use a valid email for external attendees.`);
    }
    if (!foundAttendees.includes(user.email.toLowerCase())) foundAttendees.push(user.email.toLowerCase());
  }

  return foundAttendees;
};

const validateMeetingTime = (meetingTime, isInstant = false) => {
  const minutesDiff = (meetingTime.getTime() - Date.now()) / 60000;
  if (minutesDiff < -5) throw new Error("That time is in the past. Please choose a future date and time.");

  if (minutesDiff < 1) {
    meetingTime.setTime(Date.now() + 2 * 60000); // Push to 2 mins from now if too close
  }
};
...
const createAutomatedMeeting = async (meetingData, userId, userEmail, platform) => {
  try {
    ...
    const isInstant = meetingData.timePreference && (meetingData.timePreference.toLowerCase().includes('now') || meetingData.timePreference.toLowerCase().includes('asap'));
    validateMeetingTime(meetingData.suggestedTime, isInstant);
...
const updateAutomatedMeeting = async (meetingId, updateFields, userId, userEmail) => {
  try {
    ...
    if (updateFields.startTime) {
      const isInstant = updateFields.startTimeText && (updateFields.startTimeText.toLowerCase().includes('now') || updateFields.startTimeText.toLowerCase().includes('asap'));
      validateMeetingTime(updateFields.startTime, isInstant);
    }
