...
const buildHTML = ({ title, startTime, endTime, organizerEmail, attendees, description, joinUrl, platform }) => {
  const COLORS = { zoom: '#2D8CFF', meet: '#34A853', google: '#34A853', teams: '#6264a7' };
  const color = COLORS[platform] || '#2D8CFF';
  const platformLabel = platform === 'zoom' ? 'Zoom' : platform === 'meet' || platform === 'google' ? 'Google Meet' : 'Meeting';
  const fmt = d => new Date(d).toLocaleString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', timeZoneName: 'short' });
